...
## Integration of \gls{andes} with Existing Data Repositories
...
Another limitation in the initial version of \gls{andes} was that it was designed to primarily support two types of instruments: CTD/Rosette and ring net deployments.
...
In a recent version release of \gls{andes}, the above limitations were addressed in the following ways:
\begin{description}

\item[Autonomy]{
The Oceanography module can now be used in complete independence from of the Ecosystem Survey module.
...
Also, the module refers to the deployments of instruments as "events" in order to be consistent with the conventions used in the \gls{elog}}

\item[Customization]{Users can decide what types of observations are collected at both the sample and event levels.
In the mission configuration, users can specify which observations and action types are should be collected for different instrument types.
Instrument types and action types are now completely user-defined.
Due to these changes, the flow of data collection for each instrument type can be managed independently.
(see Figure \@ref(fig:screenshot-oceanography-instrument-type-configuration))}

\item[Flexibility]{The order of events within an oceanographic sample can be readily shifted.}
