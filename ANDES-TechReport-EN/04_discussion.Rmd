...
The software has also been adopted as the main logging software used during ecosystem surveys conducted by DFO Gulf, Maritimes and Québec Regions.
...
Two types of reports are available for \gls{andes}: 1) reports meant to be used during ongoing field activities and 2) reports meant to be used after field activities are completed.
...
While this has some important advantages, exposing an IP to the internet comes with security risks that have to be mitigated.
...
However, as installing an ad-hoc network for scientific needs in a vessel may lead to code violations, we urge \gls{andes} users to collaborate with vessel management bodies like the \gls{CCG} or chartering authority.
...
While these features will help improve the quality of data collected, it is important that if utilized, they should be detailed explicitly in the survey sampling programme.
For example, if project leads add certain catch items to the restrict-list, this should be mirrored in the survey sampling programme.
...
\gls{andes} was first trialed in parallel to ELOG on a dedicated oceanographic survey led by the Maritimes Region \gls{azmp} in spring 2022, and was recently used as the sole event logger during the 2023 \gls{azomp} survey.
During its initial trial on the 2022 spring \gls{azmp} survey, a number of limitations were noted.
ELOG assigns each gear deployment a sequential 3-digit 'event' number.
In contrast, \gls{andes} groups all gear deployments in relation to a \gls{Set} number, which is a primarily fisheries-related term that is not applicable to oceanographic survey data collection.
However, \gls{andes} could easily be adapted to the \gls{azmp}'s normal metadata naming convention by renaming \gls{Set} to 'station', given that multiple sampling events normally occur at the same \gls{azmp} station.

The initial version of \gls{andes} included only two oceanographic gear options: CTD/Rosette and ring net deployments.
However, Argo floats, oceanographic buoys, moorings, multinet, and other gear are routinely deployed during \gls{azmp} and \gls{azomp} surveys, and the order of operations must be flexible.
In previous versions of \gls{andes}, the order of operations under each \gls{Set} could not be modified between stations.
With developer support, \gls{andes} was modified to allow for the inclusion of additional gear options and the ability to toggle and select between gear types should the order of operations change between stations.


ELOG allows users to create various 'logbooks' that allow for custom recording of metadata.
On \gls{azmp} surveys, a custom ELOG logbook was created to capture metadata related to samples collected from underway or flow-through thermosalinograph (TSG) systems used on these surveys.
A sub-module could be developed in the future to allow for a similar level of data capture using \gls{andes}.


\gls{andes} shows great promise for enhanced data recording and oceanographic sample tracking relative to ELOG.
Future modifications of \gls{andes} could incorporate information on water budget requirement for each CTD cast, and could allow for the tracking of laboratory post-processing.
...
Andes has been successfully used by a number of DFO regions and has proved to be a useful data entry application that fulfills its intended design goals.
...
# Acknowledgments
We thank all DFO and \gls{CCG} personnel who were involved in the early testing and deployment of \gls{andes} prior to and during the 2019 southern Gulf of St.
Lawrence ecosystem survey.
