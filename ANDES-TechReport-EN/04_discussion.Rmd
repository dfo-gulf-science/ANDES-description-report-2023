...
If a vessel is located within range of cellular phone signals, it is possible to make \gls{andes} accessible via the internet. This has some important advantages however exposing an IP to the internet always come with security risks that have to be mitigated.
...
The addition of allow-lists and restrict-lists was an important addition to Andes that came out of a Regionalization workshop that took place in DEC? 2022 at the Gulf Fisheries Center. While these feature will help improve the quality of data collected, it is important that if implemented, they should be detailed explicitly in the survey sampling programme. For example, if project leads add certain catch items to the restrict-list, this should be mirrored in the survey sampling programme.


While there is always an effort to make the user interface intuitive and self-explanatory, there is still the need to maintain documentation for the project. The Andes docs are maintained through GitHub Pages and are publicly available here: [https://dfo-gulf-science.github.io/andes/](https://dfo-gulf-science.github.io/andes/)
