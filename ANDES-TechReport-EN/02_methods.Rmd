...
The two situations will be explored separately in detail below.
In both cases Andes is remains a web-application that can serve multiple clients, it needs to be supported by web-server and connects to a database to store the data.

<!-- in mission -->
In the \gls{Mission} context, the web-server serves clients (input stations) connected over the vessel's \gls{LAN}.
A database service that specifically serves the Andes web-application is needed.
Since only a limited number of web-server and database connections are expected, the demands are such that the same hardware can be used to support the full application stack.

The database instance will typically be as lightweight as permitted to strictly support the ongoing mission.
We will discuss how to initialize the database state for the in section [deployment preparation] (#deploy_prep) below.

A GPS feed is expected, either by a RS-232 serial connection to the vessel \gls{NMEA} network or by a data stream over \gls{LAN}.
This will allow for the automatic acquisition of positional data for specific activities.
Optional data connections to other vessel sensors (e.g., Sonar, Scanmar) can also be configured.
...
### Deployment preparation {# deploy_prep}
