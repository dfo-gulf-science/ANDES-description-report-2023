...
As the starting point of \gls{andes} was the \gls{ese} which was used during the annual September survey conducted by \gls{dfo} Gulf Region, the replacement data entry software had to first replicate all the proven functional capabilities of the \gls{ese}. Early versions of \gls{andes} achieved those goals and the system was further developed over its usage by scientists in the Gulf Region.

The genesis of \gls{andes} was to support data collection during commercial port sampling activities [@Benoit-Daigle-2007]. To provide an alternative to the paper-based forms used, an application that could deployed on field tablets for scientific staff that collect data related to commercial fishing activities was developed. A typical usage case for such an application is to obtain length-frequency samples from commercial fisheries landings, either onboard fishing vessels, on wharves or in fish processing plants. Sampling protocols often include additional requirements such as length-stratified sampling of hard parts such as otoliths, or the collection of whole individuals for later processing in the laboratory. The data structure, protocol flexibility, and deployment requirements of this initial port sampling application provided many of the foundational components for other usage cases.
...
\item[Specimens]{The application should flag specimens whose length falls outside an acceptable range. Similarly, a validation of the specimen's length-to-weight ratio should be performed to warn users if a recorded weight falls outside the expected range for the measured length.}
