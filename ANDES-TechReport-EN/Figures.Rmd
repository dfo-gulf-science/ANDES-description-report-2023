...
(ref:oceanography-instrument-type-configuration) A screenshot portraying how the configuration of "oceanographic events" associated with a certain type of instrument can be customized. In this example, the configuration for plankton nets are being displayed. The box on above contains a list of user-defined "actions types" that will be associated with the event. Action types can be set to optional or mandatory. The lower box displays the list of user-defined fields that will be collected for all plankton nets. Fields can be set to be optional or mandatory and default values can be specified.
...
(ref:screenshot-shrimps) A screenshot displaying the Shrimps module user interface. This is the page where baskets of shrimps are entered and subsequently detailed according to their various protocols.
