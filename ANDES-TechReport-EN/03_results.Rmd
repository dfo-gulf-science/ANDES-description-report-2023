...
\gls{andes} successfully implements a suite of quality control checks. In addition to those listed in the section above, here are some additional checks that are performed. All quality control flags that are raising during data entry are appended to a report and require sign off before a set can be closed.
...
\gls{andes} will flag when the weights entered for baskets in kilograms are unreasonable. This is determined by either the default maximum basket weight listed for that mission, or the maximum basket weight for a given species, if available via the sampling requirement. \gls{andes} will also flag when the difference of total weight of *sampled* basket differs by more than 25% from the total calculated specimen weight. The total calculated specimen weight is combination of actual weights (when collected) and those which were estimated from length measurements. The latter is achieved by plugging in the observed length and regression coefficients fitted from historical data into the following equation:
...
\gls{andes} offers the option to specify regression coefficients for males, females or unspecified individuals (as shown in Figure \@ref(fig:ab_form)).
...
In addition to the meeting the requirements outlined in the section above, \gls{andes} will flag when there is a mismatch between fish maturity and somatic length.
Project leads can specify mature length thresholds for males, females or unspecified individuals in the sampling requirement of a given species (as shown in Figure \@ref(fig:mature_length_form)).

#### Observations

When entering an observation for an observation type that has categories, Andes will display the list of options to the end user and inhibit users from entering invalid selections. Andes will also ensure observation types are not left blank, however NaN entries are permitted when a particular observation is meant to be skipped. Certain observation types are meant to be unique, e.g., unique tag number. If this is flagged at the level of the observation type, end users will be notified if there is a violation of this structure.
