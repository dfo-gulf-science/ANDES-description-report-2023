...
The Andes application itself is a Django web framework project. It contains multiple Django apps that loosely correspond to the different use cases defined above. In addition to the standard templates which are used to render HTML webpages to end-users, the web framework also contains an elaborate \gls{REST-API} component. Django uses an \gls{ORM} to handle the data layer, and drivers exist for a variety of modern relational database management systems (i.e., PostgreSQL, MariaDB, MySQL, Oracle, SQLite, etc.). Django's web frontend can contain reactive components implemented in VUEjs.
...
### Backups Capability and Redundancy
