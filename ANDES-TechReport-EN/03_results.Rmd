...
The Shrimps module is an extension of the ecosystem survey module focused exclusively on shrimps as it has different workflow requirements. This module allows the user to subsample a total catch of shrimp into different species and maturity stages before collecting biological data (cephalothorax length for all species and occasionally weight for *Pandalus borealis*).
...
The user is also presented with option to include an empirical growth model together with weight vs length scatter data. This option, which only applies for ``official'' length and weight observation types, can be useful for tuning the tolerance band used for quality control (see Quality Control section below). 
...
The various tables of the Port Sampling module and how they relate to one another is shown in Figure \@ref(fig:erd-port).
