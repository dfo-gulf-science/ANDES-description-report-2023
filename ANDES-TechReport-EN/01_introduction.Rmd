...
The scientific activities conducted by the Atlantic regions of \gls{dfo} include ecosystem surveys, sentinel fisheries programs, coastal surveys, port sampling and many others where filed data is collected. 
...
To support the data collection during those surveys, paper-based systems were initially used during field work on research vessels, fishing vessels or at commercial ports. 
...
- While these applications were built by \gls{dfo}, multiple attempts to obtain support for these tools from \gls{imts} were unsuccessful. This lack of support means that technicians often resort to either Excel or pen and paper to perform activities unsupported by the applications. Performing data entry using separate and disconnected methods means that the data has to be manually consolidated at a later date which is time-consuming and prone to error.
...
It provides guidelines for assessing the suitability of \gls{andes} for novel applications (e.g., other field activities and sampling programs) and discusses the ways in which the application could gain further adoption in supporting data collections activities within \gls{dfo}.
