...
- \gls{vb}, the main development framework used to build the applications, are dated programming languages. To adapt the software so that modern libraries can be used and to develop an \gls{api} in \gls{vb} will become increasingly difficult over time.
...
- While these applications were built by \gls{dfo}, multiple attempts to obtain support for these tools from \gls{imts} (now \gls{cdos}, \gls{cdos}) were unsuccessful. This lack of support means that technicians often resort to either using a spreadhseet application such as Excel, or use a pen and paper to perform activities unsupported by the applications. Performing data entry using separate and disconnected methods means that the data has to be manually consolidated at a later date which is time-consuming and prone to error.
...
This project effectively began in the summer of 2018 as an application to be deployed on field tablets to support port sampling data collection, and later evolved into an application with a wider range of uses. 
